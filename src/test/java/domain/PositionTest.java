...
public class PositionTest {
    ...
    public void testToAndFromDocument() {
        Date date = new Date();
        Position position = new Position(Long.parseLong("1"), date, 5.0, 7.0);
        Position positionConverted = Position.fromDocument(position.toDocument());
        assertSame(date, positionConverted.getDate());
        Assert.assertEquals(date, positionConverted.getDate());
        Assert.assertEquals(5.0, positionConverted.getLongitude());
        Assert.assertEquals(7.0, positionConverted.getLatitude());
    }
}
